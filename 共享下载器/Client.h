...
public:
	...
	bool GetonlineHost()
	{
		...
		if (ch == 'Y')
		{
			...
			//1.获取网卡信息，进而获得所有局域网中所有IP地址列表信息，最终得到在线主机列表（主机配对三步走：）
			std::vector<Adapter> list;
			AdapterUtil::GetAllAdapter(&list);

			//（1）获取所有主机IP地址，添加到host_list中
			...
			for (int i = 0; i < list.size(); ++i)//得到所有主机IP地址列表
			{
				...
				//计算网络号
				uint32_t net = (ntohl(ip & mask));    //将(大端）网络字节序转化为(小端)主机字节序

				//计算最大主机号
				uint32_t max_host = (~ntohl(mask));      
				for (int j = 1; j < (int32_t)max_host; ++j)
				{
					uint32_t host_ip = net + j;  //主机的IP地址：由网络号和主机号相加
					Host host;
			...
			//for循环进行完，则获取到所有的主机IP地址

			//（2）对Host_list中主机创建线程进行配对
			...
			//（3）等待所有主机线程配对完毕，判断配对结果，将在线主机添加到_online_host中
			for (int i = 0; i < host_list.size(); ++i)
			{
				thr_list[i]->join();
...
private:
	std::vector<Host> _online_host;   //在线的主机列表
};
